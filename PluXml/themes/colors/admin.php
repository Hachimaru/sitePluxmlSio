...
if(!empty($_POST)) {
	...
	$plxPlugin->setParam('illustration', $_POST['illustration'], 'cdata');
	$plxPlugin->setParam('about', $_POST['about'], 'cdata');
	$plxPlugin->setParam('coordonnees', $_POST['coordonnees'], 'cdata');
...
<div data-tab="tab5" ng-app="angularApp2" ng-controller="appController2">
	...
	<p>
		<label for="coordonnees"><?php $plxPlugin->lang('L_FORM_COORDONNEES') ?></label>
		<textarea ng-model="content" id="coordonnees" rows="10"   name="coordonnees"><? echo $plxPlugin->getParam('coordonnees'); ?></textarea>
...
<div data-tab="tab6">
	...
	<p>
		<label for="illustration">
			<?php $plxPlugin->lang('L_FORM_ILLUSTRATION') ?>
			<a id="toggler_thumbnail" href="javascript:void(0)" onclick="mediasManager.openPopup('illustration', true)">+</a>
		</label>

		<input id="illustration" name="illustration"  maxlength="255" value="<?php echo plxUtils::strCheck($plxPlugin->getParam("illustration")) ?>">
	</p>

	<?php 

		$illustration = $plxPlugin->getParam("illustration");

			if (empty($illustration)) {?>

			<img src="<?php echo PLX_PLUGINS ?>colors/app/AucuneImage.jpg" alt="illustration">

				<?php
			}else{
				
				$imgUrl = PLX_ROOT.$illustration;

				?>

			<img src="<?php echo $imgUrl; ?>" alt="illustration">
...
app2.controller("appController2", function($scope2, $sce2){
	$scope2.content = "<? echo $plxPlugin->getParam('coordonnees'); ?>";
